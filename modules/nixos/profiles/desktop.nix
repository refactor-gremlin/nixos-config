...
{ config, lib, ... }: {
  options.myConfig.profiles.desktop.enable = lib.mkEnableOption "Desktop profile (HyDeNix, portals, audio, bluetooth, locale, boot, disk, maintenance)";

  config = lib.mkIf config.myConfig.profiles.desktop.enable {
    myConfig.desktop.hydenix.enable = true;
